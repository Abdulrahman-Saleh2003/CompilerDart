...
public class BaseVisitor extends DartParserBaseVisitor<ASTNode> {
    ...
    public ASTNode visitProgram(DartParser.ProgramContext ctx) {
        ...
        // Ø·Ø¨Ø§Ø¹Ø© Ø¬Ø¯ÙˆÙ„ Ø§Ù„Ø±Ù…ÙˆØ² ÙÙŠ Ù†Ù‡Ø§ÙŠØ© Ù…Ø±Ø­Ù„Ø© Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ù…Ø«Ù„ Ø§Ù„Ù†Ù…ÙˆØ°Ø¬
        symbolTable.print();
    ...
    public ASTNode visitClassDeclaration(DartParser.ClassDeclarationContext ctx) {
        ...
        ClassDeclaration classDecl = new ClassDeclaration(modifier, name, typeParams, superclass, mixins, interfaces, members);

        // Ø¥Ø¶Ø§ÙØ© Ø§Ù„ÙƒÙ„Ø§Ø³ Ø¥Ù„Ù‰ Ø¬Ø¯ÙˆÙ„ Ø§Ù„Ø±Ù…ÙˆØ²
        Row row = new Row();
        row.setName(name);
        row.setType("Class");
        row.setVarType(modifier != null ? modifier : "class");
        row.setValue(classDecl);
    ...
    public ASTNode visitMixinDeclaration(DartParser.MixinDeclarationContext ctx) {
        ...
        MixinDeclaration mixinDecl = new MixinDeclaration(name, typeParams, onTypes, interfaces, members);

        Row row = new Row(name, "Mixin", "mixin", mixinDecl);
        symbolTable.getRows().add(row);
    ...
    public ASTNode visitExtensionDeclaration(DartParser.ExtensionDeclarationContext ctx) {
        ...
        ExtensionDeclaration extDecl = new ExtensionDeclaration(name, typeParams, onType, members);
        if (name != null) {
            symbolTable.getRows().add(new Row(name, "Extension", "extension", extDecl));
        }
        return extDecl;
    }
    ...
    public ASTNode visitEnumDeclaration(DartParser.EnumDeclarationContext ctx) {
        ...
        EnumDeclaration enumDecl = new EnumDeclaration(name, typeParams, interfaces, entries, members);

        Row row = new Row(name, "Enum", "enum", enumDecl);
        symbolTable.getRows().add(row);
    ...
    public ASTNode visitTypedefDeclaration(DartParser.TypedefDeclarationContext ctx) {
        ...
        TypedefDeclaration typedefDecl = new TypedefDeclaration(name, typeParams, type);

        Row row = new Row(name, type.toString(), "Typedef", typedefDecl);
        symbolTable.getRows().add(row);
    ...
    public ASTNode visitFieldDeclaration(DartParser.FieldDeclarationContext ctx) {
        ...
        for (DartParser.VariableDeclaratorContext vd : ctx.variableDeclarator()) {
            VariableDeclarator declarator = (VariableDeclarator) visit(vd);
            declarators.add(declarator);

            // Ø¥Ø¶Ø§ÙØ© ÙƒÙ„ Ø­Ù‚Ù„ Ø¥Ù„Ù‰ Ø¬Ø¯ÙˆÙ„ Ø§Ù„Ø±Ù…ÙˆØ²
            Row row = new Row();
            row.setName(declarator.name);
            row.setType(type != null ? type.toString() : "var");
            String varType = "Field";
            if (isStatic) varType = "Static Field";
            if (isFinal) varType += " final";
            if (isConst) varType += " const";
            row.setVarType(varType);
            row.setValue(declarator.initializer);
            symbolTable.getRows().add(row);
        }
    ...
    public ASTNode visitConstructorDeclaration(DartParser.ConstructorDeclarationContext ctx) {
        ...
        ConstructorDeclaration constructor = new ConstructorDeclaration(isConst, isFactory, name, parameters, initializers, body);

        Row row = new Row(name, "Constructor", isFactory ? "factory" : (isConst ? "const" : "constructor"), constructor);
        symbolTable.getRows().add(row);
    ...
    public ASTNode visitMethodDeclaration(DartParser.MethodDeclarationContext ctx) {
        ...
        MethodDeclaration method = new MethodDeclaration(isStatic, returnType, name, parameters, isAsync, body, arrow);

        Row row = new Row();
        row.setName(name);
        row.setType(returnType != null ? returnType.toString() : "void");
        row.setVarType(isStatic ? "Static Method" : "Method");
        row.setValue(method);
        symbolTable.getRows().add(row);
    ...
    public ASTNode visitGetterDeclaration(DartParser.GetterDeclarationContext ctx) {
        ...
        Block body = ctx.block() != null ? (Block) visit(ctx.block()) : null;
        Expression arrow = ctx.expression() != null ? (Expression) visit(ctx.expression()) : null;
        GetterDeclaration getter = new GetterDeclaration(isStatic, returnType, name, body, arrow);

        symbolTable.getRows().add(new Row(name, returnType != null ? returnType.toString() : "dynamic", "Getter", getter));
    ...
    public ASTNode visitSetterDeclaration(DartParser.SetterDeclarationContext ctx) {
        ...
        Block body = ctx.block() != null ? (Block) visit(ctx.block()) : null;
        Expression arrow = ctx.expression() != null ? (Expression) visit(ctx.expression()) : null;
        SetterDeclaration setter = new SetterDeclaration(isStatic, name, param, body, arrow);

        symbolTable.getRows().add(new Row(name, "void", "Setter", setter));
    ...
    public ASTNode visitOperatorDeclaration(DartParser.OperatorDeclarationContext ctx) {
        ...
        Block body = ctx.block() != null ? (Block) visit(ctx.block()) : null;
        Expression arrow = ctx.expression() != null ? (Expression) visit(ctx.expression()) : null;
        OperatorDeclaration opDecl = new OperatorDeclaration(returnType, op, params, body, arrow);

        symbolTable.getRows().add(new Row("operator " + op, returnType != null ? returnType.toString() : "void", "Operator", opDecl));
    ...
    public ASTNode visitTopLevelFunctionDeclaration(DartParser.TopLevelFunctionDeclarationContext ctx) {
        ...
        Block body = ctx.block() != null ? (Block) visit(ctx.block()) : null;
        Expression arrow = ctx.expression() != null ? (Expression) visit(ctx.expression()) : null;
        FunctionDeclaration function = new FunctionDeclaration(returnType, name, parameters, isAsync, body, arrow);

        Row row = new Row();
        row.setName(name);
        row.setType(returnType != null ? returnType.toString() : "void");
        row.setVarType("Function");
        row.setValue(function);
        symbolTable.getRows().add(row);
    ...
    public ASTNode visitTopLevelVariableDeclaration(DartParser.TopLevelVariableDeclarationContext ctx) {
        ...
        for (DartParser.VariableDeclaratorContext vd : ctx.variableDeclarator()) {
            ...
            row.setName(declarator.name);
            row.setType(type != null ? type.toString() : "var");
            String varType = "Global Variable";
            if (isFinal) varType += " final";
            if (isConst) varType += " const";
            row.setVarType(varType);
            row.setValue(declarator.initializer);
    ...
    public ASTNode visitForStatement(DartParser.ForStatementContext ctx) {
        ...
        if (flp.IN() != null) {
            ...
            Row row = new Row(itemName, itemType != null ? itemType.toString() : "var", "Loop Variable", null);
            symbolTable.getRows().add(row);
            ...
            if (flp.forInitializer() != null) {
                if (flp.forInitializer().variableDeclarator() != null && !flp.forInitializer().variableDeclarator().isEmpty()) {
                    ...
                    for (DartParser.VariableDeclaratorContext vd : flp.forInitializer().variableDeclarator()) {
                        ...
                        Row row = new Row(declarator.name, t != null ? t.toString() : "var", "Loop Variable", declarator.initializer);
                        symbolTable.getRows().add(row);
                    }
    ...
    public ASTNode visitTryCatchStatement(DartParser.TryCatchStatementContext ctx) {
        ...
        for (DartParser.OnPartContext op : ctx.onPart()) {
            ...
            if (op.catchPart() != null) {
                ...
                symbolTable.getRows().add(new Row(errVar, onT != null ? onT.toString() : "Exception", "Catch Variable", null));
                if (op.catchPart().IDENTIFIER().size() > 1) {
                    stVar = op.catchPart().IDENTIFIER(1).getText();
                    symbolTable.getRows().add(new Row(stVar, "StackTrace", "Catch Variable", null));
                }
            }
    ...
    private List<Parameter> extractParameters(DartParser.FormalParameterListContext fpl) {
        ...
        for (DartParser.NormalParameterContext np : pl.normalParameter()) {
            Parameter p = toParameter(np);
            params.add(p);
            // Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ù…Ø¹Ø§Ù…Ù„Ø§Øª Ø¥Ù„Ù‰ Ø¬Ø¯ÙˆÙ„ Ø§Ù„Ø±Ù…ÙˆØ²
            symbolTable.getRows().add(new Row(p.name, p.type != null ? p.type.toString() : "var", "Parameter", p.defaultValue));
        }
        if (pl.optionalParameterGroup() != null) {
            ...
            for (DartParser.OptionalPositionalParameterContext op : opg.optionalPositionalParameter()) {
                Parameter p = toOptionalPositionalParameter(op);
                params.add(p);
                symbolTable.getRows().add(new Row(p.name, p.type != null ? p.type.toString() : "var", "Optional Parameter", p.defaultValue));
            }
            for (DartParser.NamedParameterContext np : opg.namedParameter()) {
                Parameter p = toNamedParameter(np);
                params.add(p);
                symbolTable.getRows().add(new Row(p.name, p.type != null ? p.type.toString() : "var", "Named Parameter", p.defaultValue));
            }
        }
        return params;
    }
